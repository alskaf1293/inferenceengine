#!/usr/bin/env python3
"""Composable RTL contract sweep for large fan-in/fan-out primitives."""
...
CASES = [
    Case("fanin_64", 64, 1),
    Case("fanin_128", 128, 1),
    Case("fanin_256", 256, 1),
    Case("backflow_64", 1, 64),
    Case("backflow_128", 1, 128),
    Case("backflow_256", 1, 256),
    Case("balanced_64", 64, 64),
    Case("relu_fanin_256", 256, 1, act="relu"),
]
...
def run_case(case: Case, out_dir: Path, tol: float, reuse_existing: bool) -> dict[str, object]:
    ...
    env = os.environ.copy()
    env["VERILATOR_MDIR"] = f"obj_dir/contract_{case.name}"
...
def main() -> None:
    ...
    out_dir = ROOT / "runs" / "rtl_contract_sweep"
    rows = [
        run_case(case_by_name[name], out_dir, args.tol, args.reuse_existing)
        for name in selected
    ]
    summary_path = ROOT / "runs" / "rtl_contract_sweep_summary.csv"
