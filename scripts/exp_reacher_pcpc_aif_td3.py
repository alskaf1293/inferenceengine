#!/usr/bin/env python3
"""Reacher TD3 -> BP-AIF -> PC/PC AIF replication sweep.

This mirrors the InvertedPendulum ladder but keeps Reacher in a separate file:
...
The PC/PC BP-equivalent lane tests the substrate-replacement claim: PC actor and
critic objects can preserve the BP-AIF update behavior before we tune the
exact-local/tick RTL approximation.
"""
...
class RunSpec:
    ...
    def csv_path(self, runs_dir: Path, seed: int, timesteps: int) -> Path:
        return runs_dir / f"reacher_td3_{self.key}_seed{seed}_{timesteps}.csv"
...
def summarize(args: argparse.Namespace, configs: tuple[RunSpec, ...]) -> None:
    RUNS_DIR.mkdir(exist_ok=True)
    summary_path = RUNS_DIR / f"reacher_td3_pcpc_summary_{args.total_timesteps}.csv"
    aggregate_path = RUNS_DIR / f"reacher_td3_pcpc_aggregate_{args.total_timesteps}.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--infotime", type=int, default=100)
